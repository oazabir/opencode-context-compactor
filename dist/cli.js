...
function isPluginInstalled(config) {
    if (!Array.isArray(config.plugin))
        return false;
    return config.plugin.some((entry) => Array.isArray(entry) && entry[0] === "opencode-context-compactor");
}
async function main() {
    ...
    if (!useDefaults) {
        const answer = await ask(cyan("  Use default settings? (keep_messages=10, mode=hybrid) [Y/n]: "));
